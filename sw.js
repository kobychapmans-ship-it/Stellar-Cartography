const CACHE_NAME='cartographica-r23.4-complete-consolidated-v7';
const CANONICAL_PATH='./index.html?rev=23.4-complete-consolidated-v7';
const APP_SHELL=[CANONICAL_PATH,'./manifest.webmanifest?v=23.4-complete-consolidated-v7','./icon-192.png','./icon-512.png','./apple-touch-icon.png'];
...
self.addEventListener('fetch',event=>{
  ...
  if(isAppNavigation){
    ...
    event.respondWith(
      fetch(canonical.href,{cache:'reload'})
  ...
  event.respondWith(
    fetch(event.request,{cache:'no-cache'})
      .then(r=>{if(r&&r.ok)caches.open(CACHE_NAME).then(cache=>cache.put(event.request,r.clone()));return r;})
